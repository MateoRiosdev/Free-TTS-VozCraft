      ...
      {opts.map(o => {
        ...
        return (
          <button key={o.val} onClick={() => onChange(o.val)} style={{
            ...
            border: active ? 'none' : (dark ? '1px solid rgba(255,255,255,0.1)' : '1.5px solid #e2e8f0'),
            transition: 'all 0.2s',
          }}>
            ...
            {Array.from({ length: 32 }).map((_, i) => {
              ...
              return (
                <div key={i} style={{
                  ...
                  animation: isPlaying && passed ? `wave-bar-mini ${0.6 + (i % 4) * 0.12}s ease-in-out infinite alternate` : 'none',
...
function HistoryItem({ item, dark, onPlay, onDelete, onDownload, onDownloadTxt, onRename, playingId, onStop, lang }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: 11, color: textSecondary, marginBottom: 5, display: 'flex', gap: 5, flexWrap: 'wrap' }}>
        <span>🗓 {fechaHora}</span>
        <span>·</span>
      ...
      <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
        ...
        <button onClick={() => onPlay(item)} style={{
          flex: '1 1 80px', padding: '6px 0', borderRadius: 7, border: 'none',
        ...
        {/* Descargar MP3 */}
        <button onClick={() => onDownload(item, 'mp3')} style={{
          padding: '6px 9px', borderRadius: 7, border: 'none',
          background: dark ? 'rgba(16,185,129,0.15)' : '#d1fae5',
          color: dark ? '#6ee7b7' : '#065f46', fontSize: 10, cursor: 'pointer',
          fontWeight: 700, fontFamily: "'DM Sans', sans-serif",
        }} title="Descargar MP3">MP3</button>

        {/* Descargar WAV */}
        <button onClick={() => onDownload(item, 'wav')} style={{
          padding: '6px 9px', borderRadius: 7, border: 'none',
          background: dark ? 'rgba(245,158,11,0.15)' : '#fef3c7',
          color: dark ? '#fcd34d' : '#92400e', fontSize: 10, cursor: 'pointer',
          fontWeight: 700, fontFamily: "'DM Sans', sans-serif",
        }} title="Descargar WAV">WAV</button>
...
// ─── Generar y descargar audio WAV/MP3 ────
async function generateAndDownloadAudio(item, format) {
  ...
  const f1 = audioCtx.createBiquadFilter(); f1.type = 'bandpass'; f1.frequency.value = 800 * effectivePitch; f1.Q.value = 3;
  const f2 = audioCtx.createBiquadFilter(); f2.type = 'bandpass'; f2.frequency.value = 2200 * effectivePitch; f2.Q.value = 4;
  ...
  const noiseSource = audioCtx.createBufferSource(); noiseSource.buffer = noiseBuf;
  const nf = audioCtx.createBiquadFilter(); nf.type = 'bandpass'; nf.frequency.value = 4000; nf.Q.value = 2;

  osc1.connect(f1); osc1.connect(f2);
  f1.connect(gainNode); f2.connect(gainNode);
  noiseSource.connect(nf); nf.connect(gainNode);
  gainNode.connect(audioCtx.destination);

  osc1.start(0); osc1.stop(estimatedDuration);
  noiseSource.start(0); noiseSource.stop(estimatedDuration);
...
export default function VozCraft() {
  ...
  const handleGenerar = async () => {
    if (!texto.trim()) { showToast(language === 'es' ? 'Por favor escribe algún texto' : 'Please enter some text', 'error'); return; }
  ...
  const handleDelete = id => {
    ...
    showToast(language === 'es' ? 'Eliminado del historial' : 'Removed from history');
  };

  const handleDownload = async (item, format) => {
    try { await generateAndDownloadAudio(item, format); showToast(`📥 ${format.toUpperCase()} descargado`); }
    catch (e) { console.error(e); showToast('Error al generar el audio', 'error'); }
  };

  const handleDownloadTxt = item => {
    downloadTranscript(item, language);
    showToast(language === 'es' ? '📄 Transcripción descargada' : '📄 Transcript downloaded');
  };
  ...
  const handleRenameConfirm = newName => {
    ...
    showToast(language === 'es' ? '✓ Nombre actualizado' : '✓ Name updated');
  };
    ...
    input.onchange = e => {
      ...
      reader.onload = ev => {
        try {
          ...
          if (Array.isArray(parsed)) { setHistory(parsed); saveHistory(parsed); showToast(`✓ ${parsed.length} entradas cargadas`); }
  ...
  const limpiarHistorial = () => { setHistory([]); saveHistory([]); showToast('Historial limpiado'); };
  ...
  return (
    <div style={{ minHeight: '100vh', width: '100%', background: bg, fontFamily: "'DM Sans', sans-serif", transition: 'background 0.3s' }}>
      ...
      <div className="vc-main-grid" style={{
        ...
        <div className="vc-right-card" style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
            <h3 style={{ margin: 0, fontSize: 15, fontWeight: 700, color: textPrimary, fontFamily: "'Sora', sans-serif" }}>
              {t.history}
              {history.length > 0 && (
          ...
          <div className="vc-history-scroll" style={{ flex: 1, overflowY: 'auto', minHeight: 280, maxHeight: '60vh' }}>
            {history.length === 0 ? (
              <div style={{
